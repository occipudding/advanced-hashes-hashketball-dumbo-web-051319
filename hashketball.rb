...
def shoe_size(name)
  game_hash.keys.each do |loc|
    if game_hash[loc][:players].include?(name)
      return game_hash[loc][:players][name][:shoe]
    end
  end
end
...
def team_colors(team_name)
  game_hash.keys.each do |loc|
    if game_hash[loc][:team_name] == team_name
...
def player_numbers(team_name)
  arr = []
  game_hash.keys.each do |loc|
    if game_hash[loc][:team_name].include?(team_name)
      game_hash[loc][:players].each do |player, stats|
        arr << stats[:number]
      end
    end
  end
